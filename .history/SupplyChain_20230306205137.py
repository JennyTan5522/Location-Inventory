...
class SupplyChain:
    ...
    def calcTotalCost(self,X,Y,s,Q):
        return min_FixedCost()+holding_Cost()+supplierDC_Cost()+transport_Cost()+lost_Sales()
    
     
    def calcFixedCost():#Term 1:Find fixed cost of each DC  
        totalFixedCost=0
        for j in range(NO_OF_DC):
            totalFixedCost+=FIXED_COST[j]*Y[j]
