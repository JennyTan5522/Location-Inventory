...
class SupplyChain:
    ...
    def calcInventoryLevelCost(self,s:list,Q:list):#TODO
        ilCostList=[]
        selfprobState0List=self.calcProbState0(s,Q)
        for j in range(self.NO_OF_DC):
            ...
            ilCostList.append(a*((b+c-d-e)*probState0List[j])+f*g*probState0List[j])
        return ilCostList
    ...
    def calcTerm3(self,Y:list,Q):
        ...
        total=0
        self.calcReorderRate
        for j in range(self.NO_OF_DC):
            # Q_MIN_OF_J, REORDERRATE !
            total+=(self.SETUP_COST[j]+self.FIXED_SHIPPING_COST[j]+(self.PURCHASING_COST_UNIT[j]*(Q[j])))*()*Y[j]
        return total
