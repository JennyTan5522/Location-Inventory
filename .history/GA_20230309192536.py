...
class GA:
    ...
    #=======================================================================================================
    def evaluateChromosome(self): #Maybe put in run
    ...
    def calcOptimalSQ(self,X,Y):
        ...
        for j in range(self.sc.NO_OF_DC):#Loop every DC to calculate
            ...
            for q0 in reversed(range(2,round(Qmin[j]))): #Loop q0, starting from q1-1 reversed=1259
                ...
                for s0 in reversed(range(1,q0)):#X only take one column try 1-1258
    ...
    def calcOptimalQ(self,X,Y):
        ...
        for j in range(self.sc.NO_OF_DC):#Loop every DC to calculate
            ...
            minValue=9999 #In case cannot found=0, then found the nearest min value
            targetQ1=None
            for q0 in range(1,round(Qmin[j])): #Loop q0, starting from q1-1 reversed=1259
                #When s=0, what is the optimal q?
                diff=self.sc.calcTotalCostForEachDC(X[j],Y[j],0,q0)-self.sc.calcTotalCostForEachDC(X[j],Y[j],0,q0)
                if diff<minValue:
                    minValue=diff
                    targetQ1=q0
                elif diff==0:
