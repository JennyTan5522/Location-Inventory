...
class Genetic_Algorithm():
    ...
    def crossOver(self):
        def rouletteWheelSelection(pop_parents):
            ...
            chromosomesSample=random.sample(random.randint(0,round(len(population)*0.8))
            ...
            for _ in range(round(len(population)*0.8)):
                #From 80% chromosomes choose
                elements=random.sample(population,2)
                selection_chromosomes.append(min(elements,key=fitness))
