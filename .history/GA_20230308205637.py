...
class GA:
    ...
    def calcOptimalSQ(self,X,Y):
        ...
        for j in range(self.sc.NO_OF_DC):
            for q0 in reversed(range(2,round(Qmin[j]))): 
                #For every q0 test for every s0
                for s0 in range(1,q0):#X only take one column
                    diff=self.sc.calcTotalCostForEachDC(X[j],Y[j],s0+1,q0)-self.sc.calcTotalCostForEachDC(X[j],Y[j],s0+1,q0)
