...
class SupplyChain:
    ...
    def calcQmin(self):
        ...
        Qmin=[]
        for j in range(self.NO_OF_DC):
                    QminCost=((self.COST_LOSE_1-self.PURCHASING_COST_UNIT[j])*self.DC_ARRIVAL_RATE_1[j]*(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j]))/(self.sc.HOLDING_COST[j]*self.sc.LEAD_TIME[j])
