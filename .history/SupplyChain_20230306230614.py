...
class SupplyChain:
    ...
    def calcProbState0(self,s:list,Q:list):
        ...
        probState0=[]
        for j in range(self.NO_OF_DC):
            probState0+=(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])/(self.DC_ARRIVAL_RATE_1[j]+(self.DC_ARRIVAL_RATE_2[j]+(Q[j]*self.LEAD_TIME[j]))*pow((1+self.LEAD_TIME[j]/self.DC_ARRIVAL_RATE_1[j]),s[j]))
