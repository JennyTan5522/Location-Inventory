...
class SupplyChain:
    ...
    def __init__(self,PROBLEM_SIZE,SITUATION_TYPE):
        ...
        for j in range(self.NO_OF_DC):
            for i in range(self.NO_OF_RETAILER):
                self.SHIPPING_COST[i][j]=random.randint(cost_list['SHIPPING_COST'][0],cost_list['HOLDING_COST'][1])
    ...
    def dcArrivalRate(self,X):
        ...
        DC_ARRIVAL_RATE_1=[]
        DC_ARRIVAL_RATE_2=[]
        #Loop DC, loop retailer (Summation of all retailers)
        #np.dot(self.RETAILER_ARRIVAL_RATE_1*X)
        for i in range(self.NO_OF_RETAILER):
            dcArrivalRate1=0
            dcArrivalRate2=0
            for j in range(self.NO_OF_DC):
