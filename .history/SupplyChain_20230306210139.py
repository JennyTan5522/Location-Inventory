...
class SupplyChain:
    ...
    def calcTotalCost(self,X,Y,s,Q):
        return self.calcFixedCost(Y)+self.calcHoldingCost(Y,s,Q)+supplierDC_Cost()+transport_Cost()+lost_Sales()
    ...
    def calcInventoryLevelCost(self,s,Q):
        pass

    def calcHoldingCost(self,Y:list,s,Q):
        '''Term 2: Calculate holding cost of each DC'''
        totalHoldingCost=0
        for j in range(self.NO_OF_DC):#TODO
            il=self.calcInventoryLevelCost(s,Q)
            totalHoldingCost+=self.HOLDING_COST[j]*(il)*Y[j]
