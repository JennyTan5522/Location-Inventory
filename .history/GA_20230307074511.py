...
class Genetic_Algorithm():
    ...
    def crossOver(self):
        ...
        def rouletteWheelSelection():
            ...
            #2. Calculate cumulative sum of fitness from 80% selected chromosomes
            cumulativeSum=np.cumsum(population) #TODO population change to 'fitness'
            #3. Generate a random number between 0 and cumulativeSum, choose the individual with cumulativeSum < fitness
        ...
        #4. Select 10% of chromosomes form roulette wheel selection
        pop_parent1=rouletteWheelSelection()
        pop_parent2=rouletteWheelSelection()
        #Ensure that the parents selected are not duplicated
        pop_parent2=rouletteWheelSelection() if pop_parent2==pop_parent2 else pop_parent2
