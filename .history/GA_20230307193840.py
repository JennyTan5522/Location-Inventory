...
GA=Genetic_Algorithm(10000,50,100,0.8,0.2,sc)
population=[GA.generateChromosome() for _ in range(self.POP_SIZE)]
X=[generateX(self.population[idx]) for idx in range(len(self.population))]
Y=[generateY(self.X[idx]) for idx in range(len(self.population))]    
print(self.population)  
print(self.X)
print(self.Y)  
#2. Select 10% of chromosomes from roulette wheel selection
n_parents=[rouletteWheelSelection() for _ in range(round(len(population)*0.1))]
