...
class SupplyChain:
    ...
    def calcProbState0(self,s,Q):
        ...
        totalProbState0=0
        for j in range(self.NO_OF_DC):
            totalProbState0+=(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])/(self.DC_ARRIVAL_RATE_1[j]+(self.DC_ARRIVAL_RATE_2[j]+(Q[j]*self.LEAD_TIME[j]))*(1+self.LEAD_TIME[j]/self.DC_ARRIVAL_RATE_1[j]))
