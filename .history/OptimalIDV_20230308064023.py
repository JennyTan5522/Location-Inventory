...
class OptimalIDV:
    ...
    def calcQmin(self):
        ...
        for j in range(self.sc.NO_OF_DC):qminCost=0
            for r in range(len(self.sc.CUSTOMER_CLASS.keys())):
                if r==0: #Priority customer
                    ...
                    print(self.sc.COST_LOSE_1-self.sc.PURCHASING_COST_UNIT[j])
                    print(self.sc.DC_ARRIVAL_RATE_1[j])
                    print(self.sc.DC_ARRIVAL_RATE_1[j]+self.sc.DC_ARRIVAL_RATE_2[j])
                    print(self.sc.HOLDING_COST[j]*self.sc.LEAD_TIME[j])
