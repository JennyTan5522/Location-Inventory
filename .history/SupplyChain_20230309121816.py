...
class SupplyChain:
    ...
    def __init__(self,PROBLEM_SIZE,SITUATION_TYPE):
        ...
        #Compare the random arrival rate of both priority and ordinary customer then put the less arrival rate 
        for i in range(len(self.NO_OF_DC)):
            if(RETAILER_ARRIVAL_RATE_2[i]<RETAILER_ARRIVAL_RATE_1[i]):#If dc arrival rate 2 fast than 1 then swap place
                temp=RETAILER_ARRIVAL_RATE_1[i]
                RETAILER_ARRIVAL_RATE_1[i]=RETAILER_ARRIVAL_RATE_2[i]
                RETAILER_ARRIVAL_RATE_2[i]=temp
        ...
        print('Arrival rate    : ',self.RETAILER_ARRIVAL_RATE_1)
