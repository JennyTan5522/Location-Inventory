...
class SupplyChain:
    ...
    def __init__(self,PROBLEM_SIZE,SITUATION_TYPE):
        ...
        self.DC_ARRIVAL_RATE=[random.randint(cost_list['ARRIVAL_RATE'][0],cost_list['ARRIVAL_RATE'][1]) for _ in range(self.NO_OF_DC)]
        ...
        self.DC_ARRIVAL_RATE_1=[20,25,30,35,40]
        self.DC_ARRIVAL_RATE_2=[25,30,35,38,45]
    ...
    def calcInventoryLevelCost(self,s,Q):#TODO
        ilCost=0
        for j in range(self.NO_OF_DC):
            a=(s[j]*self.DC_ARRIVAL_RATE_2[j])/(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])
            b=(self.LEAD_TIME[j]*Q[j]*(Q[j]+2*s[j]+1))/2*(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])
            c=(Q[j]*self.DC_ARRIVAL_RATE_1[j]/(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j]))
            d=(self.DC_ARRIVAL_RATE_1[j]*self.DC_ARRIVAL_RATE_2[j])/(self.LEAD_TIME[j]*(self.DC_ARRIVAL_RATE_1[j]*self.DC_ARRIVAL_RATE_2[j]))

            ilCost+=pow((1+self.LEAD_TIME[j]/self.DC_ARRIVAL_RATE_1[j]),s[j])+()

    def calcTerm2(self,Y:list,s,Q):
        '''Term 2: Calculate holding cost of each DC'''
        totalHoldingCost=0
        self.probState0=self.calcProbState0()
    ...
    def calcProbState0(self,s,Q):
        ...
        totalProbState0=0
        for j in range(self.NO_OF_DC):
            totalProbState0+=(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])/(self.DC_ARRIVAL_RATE_1[j]+(self.DC_ARRIVAL_RATE_2[j]+(Q[j]*self.LEAD_TIME[j]))*pow((1+self.LEAD_TIME[j]/self.DC_ARRIVAL_RATE_1[j]),s[j]))
        return totalProbState0
        
    def calcReorderRate(self,s,Q):
        totalReorderCost=0
        totalProbState0=self.calcProbState0(s,Q)
        for j in range(self.NO_OF_DC):
            totalReorderCost+=(self.DC_ARRIVAL_RATE_1[j]+self.DC_ARRIVAL_RATE_2[j])*totalProbState0*(s[j]+1)
    ...
    def calcTerm5(self,Y:list):
        ...
        for j in range(self.NO_OF_DC):
            for r in range(self.CUSTOMER_CLASS): #CUSTOMER CLASS !
                #Calculate mean shortage rate 1
                if (r==0):
