...
class SupplyChain:
    ...
    def __init__(self,PROBLEM_SIZE,SITUATION_TYPE):
        ...
        self.RETAILER_ARRIVAL_RATE_1=[random.randint(cost_list['RETAILER_ARRIVAL_RATE'][0],cost_list['RETAILER_ARRIVAL_RATE'][1]) for _ in range(self.NO_OF_DC)]
        self.RETAILER_ARRIVAL_RATE_2=[random.randint(cost_list['RETAILER_ARRIVAL_RATE'][0],cost_list['RETAILER_ARRIVAL_RATE'][1]) for _ in range(self.NO_OF_DC)]
    ...
    #Find the arrival rate of each DC 
    #TODO need access GA X
    #Find DC arrival rate
    #Got possible dc arrival rate same as retailer arrival rate?
    def dcArrivalRate(self,X:list):
        '''Find DC arrival rate'''
        DC_ARRIVAL_RATE_1=[]
        DC_ARRIVAL_RATE_2=[]
        for i in range(self.NO_OF_RETAILER-1):
            dcArrivalRate1=0
            dcArrivalRate2=0
            for j in range(self.NO_OF_DC):
                print(X[i][j])
                dcArrivalRate1+=self.RETAILER_ARRIVAL_RATE_1[i]*X[i][j]
                dcArrivalRate2+=self.RETAILER_ARRIVAL_RATE_2[i]*X[i][j]
            DC_ARRIVAL_RATE_1.append(dcArrivalRate1)
            DC_ARRIVAL_RATE_2.append(dcArrivalRate2)
    ...
    def calcTotalCostForEachDC(self,X:list,Y,s,Q): #X is matrix, only pass that column(ONLY COL), Y only 1 or 0 not list d
        X=[1,0,0,1,0] #Eg: status of DC1 of each retailer status, R1,R4 assign to DC1; R2,R3,R5 not assign to DC1
        Y=1 #Got retailers (which are R1,R4) assign to this DC
        fixedCost=0
        #Term 1: Calc fixed cost
        if(Y==1):
            fixedCost=self.FIXED_COST #How to know it is comes from DC1,2,3?
        #Term 2: Calc holding cost at DC
        holdingCost=
