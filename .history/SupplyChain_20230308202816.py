        ...
        for j in range(self.NO_OF_DC):
            ...
            for r in range(len(self.CUSTOMER_CLASS.keys())):
                if r==0: #Priority customer
                    ...
                    QminCost=((self.COST_LOSE_2-self.PURCHASING_COST_UNIT[j])*self.DC_ARRIVAL_RATE_1[j]*(self.sc.DC_ARRIVAL_RATE_1[j]+self.sc.DC_ARRIVAL_RATE_2[j]))/(self.sc.HOLDING_COST[j]*self.sc.LEAD_TIME[j])
