...
class SupplyChain:
    ...
    def calcTerm2(self,Y:list,s,Q):
        ...
        totalHoldingCost=0
        il=self.calcInventoryLevelCost(s,Q) #If s and Q are list TODO
        for j in range(self.NO_OF_DC):
            totalHoldingCost+=self.HOLDING_COST[j]*(il[j])*Y[j]
        return totalHoldingCost
    
    def calcTerm3(self):
        '''Term 3: Calculate setup cost, unit purchasing cost and fixed'''
        total=0
        for j in range(self.NO_OF_DC):
            # Q_MIN_OF_J, REORDERRATE !
            total+=self.SETUP_COST[j]+self.FIXED_SHIPPING_COST[j]+self.PURCHASING_COST_UNIT[j]*(Q_MIN_OF_J[I])*(REORDERRATE)*Y[j]
